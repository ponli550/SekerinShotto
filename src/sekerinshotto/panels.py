...
def render(view: str, con, content: Path | None, state_root: Path, category: str | None = None) -> str:
    ...
    if view == "home":
        ...
        for iid, rec, cat, cap, text in rows:
            r = json.loads(rec)
            out.append(f"  {iid.split(':')[1][:8]} {(cap or '')[5:10]} {cat[:9]:<9} {headline(r, text, words)}")
    elif view == "notes":
...
def keys_for(name: str) -> str:
    ...
    if name == "ss-class":
        ...
        rows = ["l\tnotes with this term → results board\tterm-side\tsekerinshotto panel set {row} && ss-results-popup"]
    elif name == "ss-groups":
