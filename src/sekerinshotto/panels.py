...
def keys_for(name: str) -> str:
    ...
    g += ["s\tsearch (excerpts are redacted)\tinput:search: |term-side|sekerinshotto search {input} --limit 50" + PAGER,
    ...
    rows: list[str] = []
    if name == "ss-class":
        rows = ["l\tlist this category (redacted)\tterm-side\tsekerinshotto list --category {row} --limit 200" + PAGER]
    elif name == "ss-concepts":
        rows = ["l\tnotes with this term (redacted)\tterm-side\tsekerinshotto search {row} --limit 100" + PAGER]
    elif name == "ss-groups":
